...
/* Device Address */
void
TWI_address (unsigned char address, unsigned char mode)
{
...
TWI_start (void)
{
  ...
  while(( ! (TWCR & (1 << TWINT) )) && ( (( (TWSR&0xf8) != 0x08)) || (((TWSR & 0xf8) != 0x10)) ) ) ; //wait for TWSR to be sure its start or repeated start
    {
      /* Why there isn't any code here ?! */
    }
}

/*Sending the Address*/
void
TWI_send_DeviceAddress_Read (unsigned char address)
{
...
void
TWI_send_DeviceAddress_Write (unsigned char address)
{
...
void
TWI_send_data (unsigned char Data, unsigned char mode)
{
  TWDR = Data ;
  TWCR = (1 << TWEN) | (1 << TWINT) ;
  while( ( ! (TWCR & (1 << TWINT) ))) ;
...
/*Receive with ACK*/
unsigned char
TWI_recieve_data_ACK (unsigned char mode)
{
  switch (mode)
    { case MASTER:
...
/* Receive with NACK */
unsigned char
TWI_recieve_data_NACK (unsigned char mode)
{
  switch (mode)
    { case MASTER:
	TWCR = (1 << TWEN ) | (1 << TWINT ) ;
